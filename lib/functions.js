...
export const maskBankNumber = (number) => {
    if (!number) return "";
  
    const str = number.toString();
    const visibleLength = 4;
    const maskedLength = Math.max(0, str.length - visibleLength);
...
if (diffInDays === 0) return "今天上線";
if (diffInDays === 1) return "1天前上線";
if (diffInDays >= 30) return "一個月前上線";
return `${diffInDays}天前上線`;
};