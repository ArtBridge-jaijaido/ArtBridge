      ...
      if (isInitialLoad) {
        ...
        switch (change.type) {
          case "modified":
            ...
            const oldData = store
              .getState()
              .painterArticle.painterArticles
              .find(a => a.articleId === newData.articleId);

            // 取得「實際改動」的欄位
            const changedKeys = getChangedKeys(newData, oldData);
            console.log("改動的欄位：", changedKeys);

            // 檢查是否只改圖片欄位， 當文章圖片改動時 前端不同步更新
            const onlyImageKeys = ["exampleImageUrl", "blurredImageUrl"];
            if (isOnlyAllowedKeysChanged(changedKeys, onlyImageKeys)) {
              console.log("只改圖片，跳過 Redux 更新");
              break;
            }
          default:
...
function getChangedKeys(newData, oldData) {
  ...
  return Object.keys(newData).filter((key) => {
    ...
    if (newVal?.toDate && typeof oldVal === "string") {
      return newVal.toDate().toISOString() !== oldVal;
    }
...
/**
 * 判斷改動欄位是否「全部」都在 allowedKeys 裡
 */
function isOnlyAllowedKeysChanged(changedKeys, allowedKeys) {
