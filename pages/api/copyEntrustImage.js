...
export default async function handler(req, res) {
  ...
  const { userId, entrustId, orderId, supplementaryImageUrls = [] } = req.body;

  if (!userId || !entrustId || !orderId) {
    return res.status(400).json({ error: "缺少參數" });
  }

  try {
    // ✅ 複製範例圖
    const examplePath = `entrustMarket/${userId}/${entrustId}/exampleImage.jpg`;
    ...
    for (let i = 0; i < supplementaryImageUrls.length; i++) {
      const sourcePath = `entrustMarket/${userId}/${entrustId}/supplementaryImage_${i + 1}.jpg`;
